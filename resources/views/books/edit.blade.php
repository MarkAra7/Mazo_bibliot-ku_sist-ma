            ...
            <div>
                ...
                <select name="author_ids[]" multiple size="4"
                    ...
                    @foreach ($authors as $author)
                        <option value="{{ $author->id }}" @selected(in_array($author->id, old('author_ids', $book->authors->pluck('id')->toArray())))>{{ $author->name }}</option>
                    @endforeach
                </select>
                @error('author_ids')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            ...
            <div>
                ...
                <select name="category_ids[]" multiple size="4"
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" @selected(in_array($category->id, old('category_ids', $book->categories->pluck('id')->toArray())))>{{ $category->name }}</option>
                    @endforeach
                </select>
                @error('category_ids')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            ...
            <div>
                ...
                <select name="branch_id"
                    ...
                    @foreach ($branches as $branch)
                        <option value="{{ $branch->id }}" @selected(old('branch_id', $book->branch_id) == $branch->id)>({{ $branch->address }}) {{ $branch->name }}</option>
                    @endforeach
                </select>
                @error('branch_id')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
