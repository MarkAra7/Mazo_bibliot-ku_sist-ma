...
@section('page_title', 'Pievienot autoru')
@section('title', 'Pievienot autoru — Mazo bibliotēku sistēma')

@section('content')
<div class="fade-in max-w-2xl">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200">
        @include('partials._form_header', ['backRoute' => 'authors.index', 'title' => 'Pievienot autoru'])
        <form action="{{ route('authors.store') }}" method="POST" class="p-6 space-y-5">
            @csrf
            <div>
            ...
            <div>
                ...
                <textarea name="bio" rows="4"
                    class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm @error('bio') border-red-300 bg-red-50 @enderror">{{ old('bio') }}</textarea>
