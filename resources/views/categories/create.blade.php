...
@section('page_title', 'Pievienot kategoriju')
@section('title', 'Pievienot kategoriju — Mazo bibliotēku sistēma')

@section('content')
<div class="fade-in max-w-2xl">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200">
        @include('partials._form_header', ['backRoute' => 'categories.index', 'title' => 'Pievienot kategoriju'])
        <form action="{{ route('categories.store') }}" method="POST" class="p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                    class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm @error('description') border-red-300 bg-red-50 @enderror">{{ old('description') }}</textarea>
